...
export function Row({ player }: Props) {
  ...
  return (
    <tr className={`${playerRank.bgClass} border-separate border-spacing-2 border-b-2 border-black`} >
      <td className="md:text-2xl text-gray-300 md:px-6 md:py-3 md:p-1 whitespace-nowrap">
        <div>{isActive && `#${player.rankedNetplayProfile.rank}`}</div>
        {Boolean(rankChange) && changeArrow(rankChange)} </td>
      <td className="text-gray-100 md:px-6 md:py-3 p-1 whitespace-nowrap text-center overflow-hidden md:max-w-full max-w-[7rem] text-elipses">
        ...
        <div className="text-gray-400 text-xs">{player.connectCode.code}</div>
      </td>
      <td className="md:text-xl text-sm text-gray-900 md:px-6 md:py-3 p-1 whitespace-nowrap text-center">
        ...
        <div className="md:text-sm text-xs max-w-xs text-gray-300 uppercase">
          {playerRank.name}
        </div>
      ...
      <td className="md:text-sm text-xs text-gray-300 md:px-6 md:py-3 py-1  md:max-w-[16rem] max-w-[3rem]">
        <Characters player={player} totalGames={totalGames} />
      </td>
      <td className="md:text-xl text-gray-300 text-sm md:px-6 md:py-3 md:p-1 whitespace-nowrap">
