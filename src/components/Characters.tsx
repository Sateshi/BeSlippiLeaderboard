...
export function Characters({ player, totalGames }: Props) {
  ...
  const condensedView = () => {
    ...
    <div className="md:mx-1 mx-0.5 p-1 rounded-full border-gray-300 md:border-2 border border-dashed md:h-10 md:w-10 h-4 w-4 text-xs flex flex-col items-center justify-center hover:border-solid hover:text-gray-500 hover:border-gray-500" onClick={expandChracters} >
      {isSm && <><div>+{characters.length - CONDENSED_SIZE}</div></>}
      {!isSm && '+'}
    </div>}
    </>
  };
  ...
  const expandedView = () => {
    ...
    <Character id={codeToId(player.connectCode.code)}
      key={c.character} totalGames={totalGames} stats={c}/>)}
    {shouldCondense &&
    <div className="md:mx-1 mx-0.5 p-1 rounded-full border-gray-300 md:border-2 border border-dashed md:h-10 md:w-10 h-4 w-4 text-xs flex flex-col items-center justify-center hover:border-solid hover:text-gray-500 hover:border-gray-500" onClick={condenseCharacters} >
      {isSm && 'Hide'}
      {!isSm && '-'}
    </div>}
    </>
